...
class COCO:
    def __init__(self,
        ...
        self.annotation_train = Annotation(name="train"+self.coco_type, ann_url=ann_train_url, old_ann_url=checkPath(old_ann_path)+dic.get("train", "."), categories=categories, appending=False)
        self.annotation_val = Annotation(name = "val"+self.coco_type, ann_url=ann_val_url, old_ann_url=checkPath(old_ann_path)+dic.get("val", "."), categories=categories, appending=False)
        self.annotation_test = Annotation(name="test"+self.coco_type, ann_url=ann_test_url, old_ann_url=checkPath(old_ann_path)+dic.get("test", "."), categories=categories, appending=False)
    ...
    def insertTrainImage(self, file_name, height=1080, width=1920):
        ...
        if img_id != -1:
            self.annotation_train.isnertImage(img_id=img_id, file_name=img_name, height=height, width=width)
        return img_id
    ...
    def insertValImage(self, file_name, height=1080, width=1920):
        ...
        if img_id != -1:
            self.annotation_val.isnertImage(img_id=img_id, file_name=img_name, height=height, width=width)
        return img_id
    ...
    def insertTestImage(self, file_name, height=1080, width=1920):
        ...
        if img_id != -1:
            self.annotation_test.isnertImage(img_id=img_id, file_name=img_name, height=height, width=width)
        return img_id
...
class Annotation:
    ...
    def isnertImage(self, img_id, file_name, height=1080, width=1920):
...
class ImageSource():
    ...
    def __init__(self, name="default", root_path=None, logging=True) -> None:
        ...
        self.logging = logging
        self.log_url = f"./img-src-{self.name}.log"
